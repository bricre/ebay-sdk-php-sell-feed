 ...
 * templates. The response consists of 0 or more sequenced pages where each page
 * has 0 or more items.
 */
class ScheduleTemplateCollection extends AbstractModel
{
     ...
     * number of schedule templates to return per page, from the result set. A result
     * set is the complete set of schedule templates returned by the method. Note:
     * Though this parameter is not required to be submitted in the request, the
     * parameter defaults to 10 if omitted. Note: If this is the last or only page of
     * the result set, the page may contain fewer tasks than the limit value. To
     * determine the number of pages in a result set, divide the total value (total
     * number of tasks matching input criteria) by this limit value, and then round up
     * to the next integer. For example, if the total value was 120 (120 total tasks)
     * and the limit value was 50 (show 50 tasks per page), the total number of pages
     * in the result set is three, so the seller would have to make three separate
     * getScheduleTemplates calls to view all tasks matching the input criteria.
     *
     * @var int
     */
    public $limit = null;
