...
/**
 * The type that defines the fields for a paginated result set of tasks. The
 * response consists of 0 or more sequenced pages where each page has 0 or more
 * items.
 */
class TaskCollection extends AbstractModel
{
    /**
     ...
     * complete set of tasks returned by the method. Note: Though this parameter is not
     * required to be submitted in the request, the parameter defaults to 10 if
     ...
     * so the seller would have to make three separate getTasks calls to view all tasks
    ...
    /**
     * An array of the tasks on this page. The tasks are sorted by creation date. An
     * empty array is returned if the filter criteria excludes all tasks.
     *
     * @var \Ebay\Sell\Feed\Model\Task[]
     */
    public $tasks = null;
